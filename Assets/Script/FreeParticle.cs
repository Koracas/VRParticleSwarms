...
public class FreeParticle : MonoBehaviour
{
	...
	void Start()
	{
		...
		for (int i = 0; i < particleCount; ++i)
		{
            ...
            //particleArray[i].position.z = Random.value * 2 - 1.0f;
	...
	void Update()
	{
		Vector3 wandPosition = controller.transform.position;

        //  Need a float[] because that is what the GPU recognizes.
		float[] wandPosition3D = { wandPosition.x, wandPosition.y, wandPosition.z };

        //float[] wandPosition3D = { 0 , 1 + Random.value , 1 + Random.value};

        // Send datas to the compute shader
        computeShader.SetFloat("deltaTime", Time.deltaTime);
		computeShader.SetFloats("wandPosition", wandPosition3D);
